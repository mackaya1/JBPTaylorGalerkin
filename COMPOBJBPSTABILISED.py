...
w_j =  2.5                     # Characteristic Rotation Speed (rotations/s)
lambda1=5.0*(r_b-r_a)/(r_a*w_j)         # Relaxation Time
...
while j < loopend:
    ...
    mu_1 = 1.0*(6.0)*10E-3            # Solvent Viscosity 
    mu_2 = 9.0*(6.0)*10E-3            # Polymeric Viscosity
    ...
    # Stress Half Step
    a3 = (2.0*We/dt)*inner(tau,R)*dx + We*(inner(dot(u12,grad(tau)),R)*dx - inner(F12, R)*dx+inner(div(u12)*tau,R)*dx)+0.00001*epstau*inner(grad(tau),grad(R))*dx
    ...
    a5=(1.0/dt)*inner(p,q)*dx+0.5*dt*inner(c0*c0*grad(p),grad(q))*dx   #Using Dynamic Speed of Sound (c=c(T,x,t))
    L5=(1.0/dt)*inner(p0,q)*dx+0.5*dt*inner(c0*c0*grad(p0),grad(q))*dx-(inner(c0*c0*rho0*div(us),q)*dx+inner(c0*c0*dot(grad(rho0),us),q)*dx)
    ...
    L4 = (We/dt)*inner(tau0,R)*dx + 2.0*(1.0-betav)*inner(sr0,R)*dx- We*(inner(dot(u12,grad(tau12)),R)*dx + inner(Fr12, R)*dx + inner(div(u12)*tau12,R)*dx ) #+ 0.00001*epstau12*inner(grad(tau0),grad(R))*dx
